...
async function bd(bid) {
  ...
  var bookdetail = JSON.parse(await rp(optb));
  var bookName = bookdetail.title;
  var coverUrl = decodeURIComponent(bookdetail.cover.replace("/agent/", ""));
  var bookAuthor = bookdetail.author;
  var bookIntro = bookdetail.longIntro;
